...
<!--todo:
    ...
    X gérer les cas d'imbrication d'app en plus des rdgGrp (et dans l'idéal, le niveau d'imbrication que l'on veut retenir) => se fait tout seul, car les app sont imbriqués dans des rdg. Et pour les niveaux d'imbrication, il suffit d'appeler le template app avec //appIl suffit de spécifier //  
    X gérer l'emplacement de la listWit, quel qu'il soit;
    X deux documents en sortie, un avec les numéros de variante, un avec le texte?
    - +: gérer witStart/witEnd, lacunaStart/lacunaEnd (en fait, pas vraiment nécessaire).
    - +: serait chic d'avoir un paramètre "negative-apparatus", defaults to false;
    - +: Créer un template intermédiaire entre 'TEI' et 'app' pour éviter la redondance au niveau du template 'TEI' ;
-->
<!-- Edit. 20170611: modifications très légères de style XSL, une reprise plus détaillée de la feuille pourrait être utile -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- On commence par définir le mode de sortie comme fichier texte, et non pas XML ou HTML ; on supprime la déclaration xml et l'indentation, qui n'ont rien à y faire-->
    ...
    <!-- La feuille est constituée de trois templates, le premier pour chaque élément TEI (c'est-à-dire une seule fois, sauf si l'on travaille avec des corpus),
    ...
    <xsl:template match="tei:listWit">
        ...
        <xsl:text>;</xsl:text>
        <xsl:if test="$printTypes"><xsl:text>VariantType;</xsl:text></xsl:if>
    ...
    <xsl:template match="tei:app">
        ...
        on compte combien d'élément du même nom précèdent dans le document, ce qui équivaut à un numéro d'ordre à l'échelle du document -->
        ...
        <xsl:if test="$printTypes"><xsl:value-of select="@type"/><xsl:text>;</xsl:text></xsl:if>
        ...
        <xsl:for-each select="ancestor::tei:TEI/descendant::tei:listWit/tei:witness">
            ...
            <!-- Pour éviter de fausses correspondances, j'ai besoin de tester si le sigle est suivi d'une espace ou est à la fin de la chaîne.
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:for-each select="$app/*"><!-- on pourrait dire lem/rdg/rdgGrp -->
                        <xsl:variable name="position" select="position()"/><!-- Celle-ci est nécessaire par contre -->
                        ...
                        <xsl:if test="descendant-or-self::element()/@wit[matches(.,$witRegex)]">
                            ...
                            <xsl:if test="count($app/*[position() > $position and .//@wit[matches(.,$witRegex)]])">
